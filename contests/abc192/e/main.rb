...
result = nil
while(current = heap.pop) do
    pointmap[current].each do |point|
        next if timemap[point] < current
        next unless trains[point]
        trains[point].each do |to, t, k|
            start = ((current.to_i % k) == 0) ? current : ((current / k) + 1) * k
            touchaku = start + t
            if !timemap[to] || timemap[to] > touchaku
                timemap[to] = touchaku
                pointmap[touchaku] ||= []
                pointmap[touchaku].push(to)
                heap.push(touchaku)
            end
        end
    end
    break if timemap[Y] && timemap[Y] < current
end
